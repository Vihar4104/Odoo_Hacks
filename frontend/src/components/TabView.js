  ...
  return (
    <ul className="tab-view flex border-b mb-6 overflow-hidden">
      {tabs.map((tab) => (
        <li
          key={tab}
          className={`relative cursor-pointer p-3 border-b-2 border-transparent text-lg transition-colors transform hover:border-gray-300 ${activeTab === tab ? 'border-green-500 bg-gray-800 text-gray-200' : 'text-gray-500'}`}
          onClick={() => onTabChange(tab)}
        >
          {tab}
          {activeTab === tab && (
            <div className="absolute bottom-0 left-0 w-full h-0.5 bg-green-500 animate-fade-in"></div>
          )}
        </li>
      ))}
      <div className={`tab-indicator absolute bottom-0 left-0 w-full h-0.5 bg-green-500 ${activeTab === tabs[0] ? 'transform translate-x-0' : 'transform translate-x-full'}`}></div>
    </ul>
  );
};
